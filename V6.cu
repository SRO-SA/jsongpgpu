...
__global__
void NewRuntime_Parallel_GPU_Batch(char* input, int length, long** output, int* size, int i) {
...
double batchMode(char* filename)
{
  ...
  for(int j=0; j<numTexts; j+=10){
    ...
    for(int i=0; i<numStreams; i++){
      ...
      //printf("i: %d\n", i);
      NewRuntime_Parallel_GPU_Batch<<<1, 1, 1000, streams[i]>>>(d_text[j+i], recordsLength[j+i], (outputs+j+i), outputsSize, i);
