...
void permute_32(uint8_t *block_d, uint32_t *permutation_output_d, uint64_t size, int total_padded_32){
    ...
    for(long i = index; i< total_padded_32; i+=stride)
    {
        ...
        permutation_output_d[i] = i!=0 ?  block_d[prev+2] | (block_d[prev+3] << 8) | (first << 16) | (second << 24) :
...
void align_right(uint32_t* permutation_output_d, uint8_t* block_d, uint32_t* prev_d, uint64_t size, int total_padded_32, int shift){
    ...
    for(long i = index; i< total_padded_32; i+=stride)
    {
        ...
        uint64_t dist = i > 0 ? ((uint64_t)((fourth << 24) | (third << 16) | (second << 8) | (first)) << 32) |
                        ((block_d[prev+3] << 24) | (block_d[prev+2] << 16) | (block_d[prev+1] << 8) | (block_d[prev]))
                        : ((uint64_t)((fourth << 24) | (third << 16) | (second << 8) | (first)) << 32) |
                        (0);
...
void check_incomplete_ascii(uint8_t* block, uint32_t* prev_incomplete_d, uint32_t* is_ascii_d, uint64_t size, int total_padded_32){
    ...
    for(long i = index; i< total_padded_32; i+=stride)
    {
        ...
        is_ascii_d[i] = (uint32_t)(((first&0b10000000) >> 7)
                    | ((second&0b10000000) >> 6)
                    | ((third&0b10000000) >> 5)
...
bool UTF8Validate(uint8_t * block_d, uint64_t size){
    ...
    //cudaEventRecord(gpu_start);
    check_special_cases<<<numBlock/2, 2*BLOCKSIZE>>>(block_d, prev1_d, sc_d, size, total_padded_32);
    cudaStreamSynchronize(0);
...
void get(uint8_t* block_d, uint32_t* output1, uint32_t* output2, uint64_t size, int total_padded_32){
    ...
    for(long i = index; i< total_padded_32; i+=stride)
    {
        ...
        for (int j = start; j<start+32 && j<size; j++){
            block_d[j] == '\\' ? res1 |= 1 << (j-start) : NULL;

            block_d[j] == '\"' ? res2 |= 1 << (j-start) : NULL;
...
uint8_t * Tokenize(uint8_t* block_d, uint64_t size, int &ret_size, uint32_t &last_index_tokens){
    ...
    */
    ...
    cudaMallocAsync(&general_ptr, total_padded_32*sizeof(uint32_t)*5,0);
