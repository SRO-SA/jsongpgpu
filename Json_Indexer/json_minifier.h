...
#ifndef JSON_MINIFIER_H
#define JSON_MINIFIER_H
#include <immintrin.h>
...
private:
  ...
  simdjson_really_inline void next(const simd8x64<uint8_t>& in, const json_block& block);
  simdjson_really_inline error_code finish(uint8_t *dst_start, size_t &dst_len);
  json_scanner scanner{};
  uint8_t *dst;
};

simdjson_really_inline void json_minifier::next(const simd8x64<uint8_t>& in, const json_block& block) {
...
simdjson_really_inline void json_minifier::step<128>(const uint8_t *block_buf, buf_block_reader<128> &reader) noexcept {
  simd8x64<uint8_t> in_1(block_buf);
  simd8x64<uint8_t> in_2(block_buf+64);
