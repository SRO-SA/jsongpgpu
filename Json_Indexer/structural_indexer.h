...
#ifndef STRUCTURAL_INDEXER_H
#define STRUCTURAL_INDEXER_H
#include "bitmanipulation.h"
#include "simd.h"
#include "json_minifier.h"
#include "utf8_lookup4_algorithm.h"

#include "find_next_document_index.h"
...
public:
  ...
  static enum error_code index(const uint8_t *buf, size_t len, dom_parser_implementation &parser, bool partial) noexcept;
...
private:
  ...
  simdjson_really_inline void next(const simd8x64<uint8_t>& in, const json_block& block, size_t idx);
...
error_code json_structural_indexer::index(const uint8_t *buf, size_t len, dom_parser_implementation &parser, bool partial) noexcept {
  if (simdjson_unlikely(len > parser.capacity())) { return error_code::CAPACITY; }
...
simdjson_really_inline void json_structural_indexer::step<128>(const uint8_t *block, buf_block_reader<128> &reader) noexcept {
  simd8x64<uint8_t> in_1(block);
  simd8x64<uint8_t> in_2(block+64);
...
simdjson_really_inline void json_structural_indexer::next(const simd8x64<uint8_t>& in, const json_block& block, size_t idx) {
...
simdjson_really_inline error_code json_structural_indexer::finish(dom_parser_implementation &parser, size_t idx, size_t len, bool partial) {
  ...
  if (unescaped_chars_error) {
    return error_code::UNESCAPED_CHARS;
  }
  ...
  if (partial) {
    ...
    if(have_unclosed_string) {
      ...
      // a valid JSON file cannot have zero structural indexes - we should have found something
      if (simdjson_unlikely(parser.n_structural_indexes == 0u)) { return error_code::CAPACITY; }
    }
    auto new_structural_indexes = find_next_document_index(parser);
    if (new_structural_indexes == 0 && parser.n_structural_indexes > 0) {
      return error_code::CAPACITY; // If the buffer is partial but the document is incomplete, it's too big to parse.
    }
